...
compute_means
***************************************************************************************/
void compute_means_zssd (unsigned char *left_image, unsigned char *right_image, int x_window_size, int y_window_size, int width, int height, double *left_means, double *right_means) {
...
void match_ZSSD_right (unsigned char *left_image, unsigned char *right_image, signed char *disparity, double *min_array, int width, int height, int x_window_size, int y_window_size, int min_disparity, int max_disparity) {
  ...
  double left_pix, right_pix, *left_means, *right_means, *left_mean_ptr, *right_mean_ptr, sum;

  left_means = (double*) CALLOC (width * height, sizeof(double));
  right_means = (double*) CALLOC (width * height, sizeof(double));
  compute_means_zssd(left_image, right_image, x_window_size, y_window_size, width, height, left_means, right_means);
  ...
  for (disp = min_disparity; disp < max_disparity; disp++) {
	...
	for (i = -x_surround; i <= x_surround; i++) 
	  for (j = -y_surround; j <= y_surround; j++) {
	    ...
	    sum +=  (right_pix - left_pix) * (right_pix - left_pix);
	  } /* for j */
...
void match_ZSSD_left (unsigned char *left_image, unsigned char *right_image, signed char *disparity, double *min_array, int width, int height, int x_window_size, int y_window_size, int min_disparity, int max_disparity) {
  unsigned int left_x;
  unsigned char *ptr_left, *ptr_right;
  double left_pix, right_pix, sum, *left_means, *right_means, *left_mean_ptr, *right_mean_ptr;
  int disp, right_lim, left_lim, y, i, j, top, bottom, left, right, x_surround, y_surround;
...
#define MATCH_RIGHT     match_ZSSD_right
#define	MATCH_LEFT	match_ZSSD_left
